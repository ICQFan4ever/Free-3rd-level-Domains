...
### Функции для работы с API Яндекс.DNS

### Создание записи
function CreateDNSRecord($domain, $token, $subdomain = '@', $type, $content)
	{
		switch($type)
			{
				case 'ns': $script = 'ns'; break;
				case 'a': $script = 'a'; break;
				case 'cname': $script = 'cname'; break;
				default: $script = 'a';
			}
		$xml = simplexml_load_file('https://pddimp.yandex.ru/nsapi/add_'.$script.'_record.xml?token='.$token.'&domain='.$domain.'&subdomain='.$subdomain.'.'.$domain.'&content='.$content);
		$result = $xml -> domains -> error;
		if($result == 'ok')
			{
				$out = true;
			}
...
### Вывод ошибки при добавлении записи
function ParseDNSResult($error = 'ok')
	{
		switch($error)
			{
				case 'ok': $result = 'Запись успешно добавлена'; break;
				case 'bad_content': $result = 'Недопустимые символы в указании адреса'; break;
				case 'record_exists': $result = 'Такая запись уже существует'; break;
				default: $result = 'Неизвестная ошибка';
			}
		return $result;
	}

### Список записей
function GetDNSRecords($domain , $token)
	{
		$xml = file_get_html('https://pddimp.yandex.ru/nsapi/get_domain_records.xml?token='.$token.'&domain='.$domain);
		$array = array();
		foreach($xml -> find('record') as $element)
			{
				$array[$element -> id] = array('type' => $element -> type, 'subdomain' => $element -> subdomain, 'content' => $element -> plaintext);
			}
		return $array;
	}

### Удаление записи
function RemoveDNSRecord($domain, $token, $id)
	{
		$xml = simplexml_load_file('https://pddimp.yandex.ru/nsapi/delete_record.xml?token='.$token.'&domain='.$domain.'&record_id='.$id);
		$result = $xml -> domains -> error;
		if($result == 'ok')
			{
				$out = true;
...
function EditDNSRecord($domain, $token, $id, $subdomain, $content, $type = 'ns')
	{
		$xml = simplexml_load_file('https://pddimp.yandex.ru/nsapi/edit_'.$type.'_record.xml?token='.$token.'&domain='.$domain.'&record_id='.$id.'&subdomain='.$subdomain.'&content='.$content);
		$result = $xml -> domains -> error;
		if($result == 'ok')
			{
				$out = true;
